...
async function runBacktest() {
    ...
    // Period: Sept 1st to Oct 31st 2025
    const startTime = new Date('2025-09-01T00:00:00Z').getTime();
    const endTime = new Date('2025-10-31T23:59:59Z').getTime();

    console.log(`🚀 STARTING SEPT-OCT 2025 BLITZ BACKTEST (NO SL, MAX ${MAX_TRADES} TRADES, AGGRESSIVE ENTRIES)`);
    console.log(`-----------------------------------------------------------------------`);
    ...
    for (let i = 50; i < minLen; i++) {
        ...
        if (activeTrades.length < MAX_TRADES) {
            for (const symbol of symbols) {
                ...
                // Using the actual updated analysis function with 'BLITZ' mode
                const result = analysis.analyzeOB(slice, { mode: 'BLITZ' });

                // Trigger entry on BULLISH or BUY (matching the new aggressive logic)
    ...
    console.log(`-----------------------------------------------------------------------`);
    console.log(`RESULTADOS POR MONEDA (SEPT-OCT 2025):`);
    symbols.forEach(s => {
