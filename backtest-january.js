...
async function runBacktest() {
    ...
    // Period: Jan 1st to Jan 30th 2026
    const startTime = new Date('2026-01-01T00:00:00Z').getTime();
    const endTime = new Date('2026-01-30T23:59:59Z').getTime();

    console.log(`🚀 STARTING JANUARY 2026 BLITZ BACKTEST (NO SL, MAX ${MAX_TRADES}, AGGRESSIVE)`);
    console.log(`-----------------------------------------------------------------------`);
    ...
    // Using min length of available data
    ...
    for (let i = 50; i < minLen; i++) {
        ...
        if (activeTrades.length < MAX_TRADES) {
            for (const symbol of symbols) {
                ...
                // Aggressive logic: Trigger on BULLISH or BUY if price <= OB High
    ...
    console.log(`-----------------------------------------------------------------------`);
    console.log(`RESULTADOS POR MONEDA (ENERO 2026):`);
    symbols.forEach(s => {
